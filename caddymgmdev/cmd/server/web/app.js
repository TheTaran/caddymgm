...
function renderSecurityTrend(container, points, key, label, className) {
  ...
  svg.innerHTML = `<path class="security-trend-grid" d="M${padding} ${padding}H${width - padding} M${padding} ${height / 2}H${width - padding} M${padding} ${height - padding}H${width - padding}"/><polyline class="${className}" points="${coordinates.join(" ")}"/>${dots}<line class="security-trend-crosshair" x1="${padding}" y1="${padding}" x2="${padding}" y2="${height - padding}" hidden/><circle class="security-trend-highlight ${className}" cx="${padding}" cy="${height - padding}" r="4" hidden/><rect class="security-trend-hover-area" x="${padding}" y="${padding}" width="${width - padding * 2}" height="${height - padding * 2}"/>`;
  container.append(svg);
  const crosshair = svg.querySelector(".security-trend-crosshair");
  const highlight = svg.querySelector(".security-trend-highlight");
  const tooltip = document.createElement("div");
  tooltip.className = "security-trend-tooltip";
  tooltip.hidden = true;
  container.append(tooltip);
  const setHoverPoint = (event) => {
    const bounds = svg.getBoundingClientRect();
    const relativeX = ((event.clientX - bounds.left) / Math.max(1, bounds.width)) * width;
    const index = points.length === 1 ? 0 : Math.max(0, Math.min(points.length - 1, Math.round((relativeX - padding) / step)));
    const x = padding + index * step;
    const y = height - padding - (values[index] / max) * (height - padding * 2);
    crosshair.setAttribute("x1", String(x));
    crosshair.setAttribute("x2", String(x));
    crosshair.hidden = false;
    highlight.setAttribute("cx", String(x));
    highlight.setAttribute("cy", String(y));
    highlight.hidden = false;
    const date = points[index].start ? new Date(points[index].start).toLocaleString() : points[index].label;
    tooltip.innerHTML = `<strong>${escapeHTML(date)}</strong><span>${values[index].toLocaleString()} ${escapeHTML(label.toLowerCase())}</span>`;
    tooltip.hidden = false;
    const xInPixels = (x / width) * bounds.width;
    tooltip.style.left = `${Math.max(58, Math.min(bounds.width - 58, xInPixels))}px`;
  };
  svg.addEventListener("pointermove", setHoverPoint);
